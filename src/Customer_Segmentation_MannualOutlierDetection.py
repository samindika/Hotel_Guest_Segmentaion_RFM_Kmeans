...
gender_distribution = df_rfm_new.groupby(
    ['cluster', 'gender']).size().unstack(fill_value=0)
...
country_cluster_pivot = df_rfm_new.pivot_table(
    index='cluster', columns='country', aggfunc='size', fill_value=0)
plt.figure(figsize=(12, 8))
sns.heatmap(country_cluster_pivot, cmap='Blues',
...
df_rfm_standard = scaler_standard.fit_transform(
    df_rfm[['Recency', 'Frequency', 'Monetary']])
df_rfm_standard = pd.DataFrame(df_rfm_standard, columns=[
                               'Recency', 'Frequency', 'Monetory'])
...
labels = model.fit_predict(
    df_rfm_standard[['Recency', 'Frequency', 'Monetory']])
centers = model.cluster_centers_
...
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
ax.scatter(df_rfm_standard['Recency'], df_rfm_standard['Frequency'],
           df_rfm_standard['Monetory'], cmap='brg', c=df_rfm_standard['cluster'])
ax.scatter(centers[:, 0], centers[:, 1],
           centers[:, 2], s=300, c='black', marker='X')
ax.set_xlabel('Recency')
ax.set_ylabel('Frequency')
...
# Calculate the silhouette score to evaluate the quality of clustering
silhout_score = silhouette_score(
    df_rfm_standard[['Recency', 'Frequency', 'Monetory']], labels)
print(silhout_score)
