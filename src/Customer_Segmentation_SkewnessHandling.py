#Data Preprocessing, Mannual outlier detection, Use of transformation techniques to handle skewness, K-Means clustering
...
# Take a copy of the df_rfm data frame
df_rfm_new = df_rfm.copy()

# Log transformation to reduce the skewness of Monetary and Frequency values
df_rfm_new['Frequency_log'] = np.log1p(
    df_rfm_new['Frequency'])  # log1p is used to avoid log(0)
df_rfm_new['Monetory_log'] = np.log1p(df_rfm_new['Monetary'])

# --------------KDE and box plots after log transformation--------
plt.figure(figsize=(20, 10))

plt.subplot(2, 2, 1)
sns.kdeplot(df_rfm_new['Frequency_log'], bw_adjust=0.5)
plt.title('Kernel Density Plot of Frequency')
plt.xlabel('Frequency')
plt.ylabel('Density')

plt.subplot(2, 2, 2)
sns.boxplot(x=df_rfm_new['Frequency_log'])
plt.title('Box Plot of Frequency')
plt.xlabel('Frequency')

plt.subplot(2, 2, 3)
sns.kdeplot(df_rfm_new['Monetory_log'], bw_adjust=0.5)
plt.title('Kernel Density Plot of Monetary')
plt.xlabel('Monetary')
plt.ylabel('Density')

plt.subplot(2, 2, 4)
sns.boxplot(x=df_rfm_new['Monetory_log'])
plt.title('Box Plot of Monetory')
plt.xlabel('Monetory')
plt.tight_layout()
plt.show()

# Calculate skewness after log transformation
skewness_4 = skew(df_rfm_new.iloc[:, 6:8])
print(skewness_4)

df_rfm_new['Frequency_boxcox'], _ = boxcox(df_rfm_new['Frequency'] + 1)
df_rfm_new['Monetory_boxcox'], _ = boxcox(df_rfm_new['Monetary'] + 1)


# ---------------KDE and box plot after boxcox transformation---------------
plt.figure(figsize=(20, 10))
plt.subplot(2, 2, 1)
sns.kdeplot(df_rfm_new['Frequency_boxcox'], bw_adjust=0.5)
plt.title('Kernel Density Plot of Frequency')
plt.xlabel('Frequency')
plt.ylabel('Density')

plt.subplot(2, 2, 2)
sns.boxplot(x=df_rfm_new['Frequency_boxcox'])
plt.title('Box Plot of Frequency')
plt.xlabel('Frequency')

plt.subplot(2, 2, 3)
sns.kdeplot(df_rfm_new['Monetory_boxcox'], bw_adjust=0.5)
plt.title('Kernel Density Plot of Frequency')
plt.xlabel('Monetary')
plt.ylabel('Density')

plt.subplot(2, 2, 4)
sns.boxplot(x=df_rfm_new['Monetory_boxcox'])
plt.title('Box Plot of Monetory')
plt.xlabel('Frequency')
plt.tight_layout()
plt.show()

# Calculate skewness after boxcox transformation
skewness_5 = skew(df_rfm_new.iloc[:, 8:10])
print(skewness_5)

scaler_minmax = MinMaxScaler()
df_rfm_minmax = scaler_minmax.fit_transform(
    df_rfm_new[['Recency', 'Frequency_boxcox', 'Monetory_boxcox']])
